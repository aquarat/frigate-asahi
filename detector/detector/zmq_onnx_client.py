...
runs inference via a pluggable backend (ONNX Runtime CPU / CoreML today,
see backends.py) and returns detection results.
...
class ZmqOnnxClient:
    ...
    def __init__(
        ...
        Args:
            endpoint: ZMQ endpoint to bind to (ipc:// or tcp://)
            model_path: Path to ONNX model file or "AUTO" for automatic model management
            providers: Explicit ONNX Runtime execution providers (implies backend "onnxruntime")
            backend: Inference backend name from backends.py ("auto", "cpu", "coreml", ...)
            models_dir: Directory where models live / get saved (default: ../models)
            threads: Intra-op thread count for CPU inference (None = runtime default)
        """
        ...
        # Inference backend (the engine seam; see backends.py)
        self.backend = create_backend(
            backend, models_dir=self.models_dir, threads=threads, providers=providers
        )
        self._stats_n = 0
        self._stats_sum_ms = 0.0
        self._stats_max_ms = 0.0
        if self.model_path != "AUTO":
            if self._load_model_file(self.model_path):
    ...
    def _load_model_file(self, model_path: str) -> bool:
        """
        Load a model file into the inference backend.

        Args:
            model_path: Path to the ONNX model file

        Returns:
            True if the backend loaded it, False otherwise
        """
        try:
            self.backend.load(model_path)
            return True
        except Exception as e:
            logger.error(f"Failed to load model into {self.backend.describe()}: {e}")
            return False
    ...
    def _load_model(self, model_name: str) -> bool:
        """
        Load a model from the models directory into the backend.
        ...
        try:
            ...
            if not self._load_model_file(model_path):
                return False
    ...
    def _run_inference(self, tensor: np.ndarray, header: dict) -> np.ndarray:
        """
        Run inference on the input tensor via the backend.
        ...
        if not self.backend.loaded:
            logger.warning("No model loaded in backend, returning zero results")
        ...
        try:
            ...
            if model_type == "dfine":
                ...
                input_name = self.backend.input_names[0]
                input_data = {input_name: tensor}

            # Run inference
            t_start = time.perf_counter()

            outputs = self.backend.infer(input_data)

            t_after_onnx = time.perf_counter()

            if model_type in (None, "yolo-generic", "yologeneric"):
    ...
    def _record_stats(self, total_ms: float, every: int = 1000):
        """Log a rolling latency summary every `every` inferences (INFO level)."""
        self._stats_n += 1
        self._stats_sum_ms += total_ms
        self._stats_max_ms = max(self._stats_max_ms, total_ms)
        if self._stats_n >= every:
            logger.info(
                f"Inference stats ({self.backend.describe()}, {self.current_model}): "
                f"n={self._stats_n} avg={self._stats_sum_ms / self._stats_n:.1f}ms "
                f"max={self._stats_max_ms:.1f}ms"
            )
            self._stats_n = 0
            self._stats_sum_ms = 0.0
            self._stats_max_ms = 0.0
            ...
            while True:
                try:
                    ...
                    if "model_request" in header:
                        ...
                        if self.model_ready and self.backend.loaded:
    ...
    def cleanup(self):
        """Clean up resources (socket, context, then the inference backend)."""
        try:
            if self.socket:
                self.socket.close()
                self.socket = None
            ...
            if backend is not None:
                backend.close()  # releases GPU objects in a safe order
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ZMQ ONNX Runtime detector for Frigate. "
        "Defaults can also be set with DETECTOR_ENDPOINT, DETECTOR_MODEL, "
        "DETECTOR_BACKEND, DETECTOR_MODELS_DIR and DETECTOR_THREADS."
    )
    parser.add_argument(
        "--endpoint",
        default=env("DETECTOR_ENDPOINT", "tcp://*:5555"),
        help="ZMQ endpoint to bind (default: tcp://*:5555; e.g. tcp://0.0.0.0:5555 "
        "or ipc:///tmp/cache/zmq_detector)",
    )
    parser.add_argument(
        "--model",
        default=env("DETECTOR_MODEL", "AUTO"),
        help="Path to ONNX model file or AUTO for automatic model management",
    )
    parser.add_argument(
        "--backend",
        default=env("DETECTOR_BACKEND", "auto"),
        choices=available_backends(),
        help="Inference backend (default: auto = coreml on Apple Silicon macOS, else cpu)",
    )
    parser.add_argument(
        "--models-dir",
        default=env("DETECTOR_MODELS_DIR"),
        help="Directory holding/receiving models (default: <repo>/models)",
    )
    parser.add_argument(
        "--threads",
        type=int,
        default=int(env("DETECTOR_THREADS", "0")) or None,
        help="Intra-op CPU threads for inference (default: runtime default; 4 is a "
        "good value on an 8-core M1 to leave room for ffmpeg)",
    )
    parser.add_argument(
        "--providers",
        nargs="+",
        default=None,
        help="Explicit ONNX Runtime execution providers (overrides --backend, "
    ...
    # systemd stops us with SIGTERM; turn it into a clean shutdown so cleanup()
    # runs (a GPU backend must release its Vulkan objects before exit).
    import signal
